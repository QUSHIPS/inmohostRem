...
class UploadHandler {
    ...
    private function get_file_objects(){
        $fotos = array();
        if(isset($_GET['bar_id']) && $_GET['bar_id'] && isset($_GET['usr_id']) && $_GET['usr_id']){
            $query = "select * from fotos_x_bar where bar_id=" . $_GET['bar_id'] . " and usr_id=" . $_GET['usr_id'];
                    ...
                    if(ftp_size($conn_id, '/fotos/' . $fila['fo_enl']) != -1 && ftp_size($conn_id, '/fotos/' . $fila['fo_enl']) > 0){
                        if(!isset($_GET['duplicar']) || !($_GET['duplicar'])){
                            ...
                            $fotos[] = array(
                                ...
                                "hash" => $hash,
                                "orden" => $fila['fo_id'],
                                "descripcion" => utf8_encode($fila['fo_det']));
                        }else{
    ...
    private function handle_file_upload($uploaded_file, $name, $size, $type, $error, $upload){
                    ...
                    if(getimagesize($uploaded_file)){
                        ...
                        if($fo->processed){
                            ...
                            if(in_array($hash, $hashs)){
                                ...
                                $login = ftp_login($conn_id,$this->options['usuario_ftp'],$this->options['password_ftp']);
                                ftp_put($conn_id, "/fotos/" . $file->name . '.jpg', '/tmp/' . $file->name . ".jpg", FTP_ASCII);
                                ftp_close($conn_id);
...
//                $file->hash = md5_file($this->options['upload_dir'] . $file->name . ".jpg");
    ...
    public function redim_foto($foto, $name){
        ...
        if($fo->uploaded){
            ...
            if($fo->processed){
                ...
                if(in_array($hash, $hashs)){
                    ...
                    $conn_id = ftp_connect($this->options['host_ftp']);
                     $login = ftp_login($conn_id,$this->options['usuario_ftp'],$this->options['password_ftp']);
                    ftp_put($conn_id, "/fotos/" . $name . '.jpg', '/tmp/' . $name . ".jpg", FTP_ASCII);
                    ftp_close($conn_id);
            ...
            foreach($upload['tmp_name'] as $index => $value){
                $info[] = $this->handle_file_upload(
                        $upload['tmp_name'][$index], isset($_SERVER['HTTP_X_FILE_NAME']) ?
                                ...
                                $_SERVER['HTTP_X_FILE_TYPE'] : $upload['type'][$index], $upload['error'][$index], $files[$index]
                );
            }
        }else{
            $info[] = $this->handle_file_upload(
