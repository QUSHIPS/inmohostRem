...
switch ($tipo_inf)
{
	...
	if ($prp_mostrar)
	{
		...
		$filtro .= " and propiedades.prp_mostrar!=4";
	}
	//referente
	if ($emp_id)
	{
		...
		<inmodb>
			...
			while ($fila=mysql_fetch_array($cons)) 
			{
				...
				<columna>
					<prp_pre><![CDATA[$fila[prp_pre]]]></prp_pre>
					<prp_dom><![CDATA[$fila[prp_dom]]]></prp_dom>
					<prp_desc><![CDATA[".substr($fila[prp_desc],0,200)."]]></prp_desc>
					<prp_nota><![CDATA[".substr($fila[prp_nota],0,200)."]]></prp_nota>
				";
				echo	$cad_ser="select * from ser_x_prp_ihost where prp_id=$fila[prp_id] and usr_id=$fila[usr_id] and ser_id=3";
					$res_ser=mysql_query($cad_ser);
					$valor=mysql_fetch_array($res_ser);
					
					echo"<dormitorios><![CDATA[$valor[valor]]]></dormitorios>";
