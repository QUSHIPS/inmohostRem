...
function armarListado(pagina_requerida, idHead){
	...
	if (pagina_requerida.readyState == 4 && (pagina_requerida.status==200 || window.location.href.indexOf("http")==-1)){
		...
		var prpPagOffset = Number(xml.getElementsByTagName('offset').item(0).firstChild.data);
		var prpPagTotal = Math.ceil(prpPagTotalPro/prpPagOffset);

		document.getElementById('prpTotalDatos').innerHTML = prpPagTotalPro;


                  if(xml.getElementsByTagName('prp_publicados_con_foto').item(0)){
                      var prp_publicados_con_foto = Number(xml.getElementsByTagName('prp_publicados_con_foto').item(0).firstChild.data);
                      var prp_visitas_totales = Number(xml.getElementsByTagName('prp_visitas_totales').item(0).firstChild.data);
                      var prp_sol_inf_totales = Number(xml.getElementsByTagName('prp_sol_inf_totales').item(0).firstChild.data);
                      var prp_sol_fot_totales = Number(xml.getElementsByTagName('prp_sol_fot_totales').item(0).firstChild.data);
                      var prp_ultima_publicacion = xml.getElementsByTagName('prp_ultima_publicacion').item(0).firstChild.data;

                      document.getElementById('prp_publicados_con_foto').innerHTML = prp_publicados_con_foto;
                      document.getElementById('prp_visitas_totales').innerHTML = prp_visitas_totales;
                      document.getElementById('prp_sol_inf_totales').innerHTML = prp_sol_inf_totales;
                      document.getElementById('prp_sol_fot_totales').innerHTML = prp_sol_fot_totales;
                      document.getElementById('prp_ultima_publicacion').innerHTML = prp_ultima_publicacion;
	...
	if(prpPagTotalPro){
		...
		for(u=1; u<=prpPagTotal; u++){
			if(u == (prpPagActual)){
				...
				prpPagTotalTemp += "<a onclick=\"cargoXML('"+prpFileListado+"', '"+idHead+"', '?orden="+prpOrdenActual+"&page="+u+"&base="+((u-1)*prpPagOffset)+"', 'listado');\" title=\"\">"+u+"</a>";
			}
			if (u != prpPagTotal) prpPagTotalTemp += " - ";
		}
		document.getElementById('prpPaginasTotales').innerHTML = prpPagTotalTemp;
		if (prpPagTotalPro <= prpPagOffset){
			document.getElementById('prpPaginacion').style.display = "none";
		} else {
			document.getElementById('prpPaginacion').style.display = "";
			document.getElementById('prpOffset').innerHTML = prpPagOffset;
...
//				Data +=" onmouseover='' onmouseover=''>";
				...
				for (var i=0; i<totalHeadMostrar; i++){
					...
					if (label == 'consulta_leida') consulta_leida = dato;	
					if (label == 'consulta_id') consulta_id= dato;
					if (label == 'fileABM') fileABM = dato;
                                        if (label == 'prp_mostrar') prp_mostrar = dato;
...
//alert('label '+label);
						...
						if(label == 'prp_citas') {
							...
							if(dato == '0') {
								...
								r=a+101;
									...
									{
										case 'mod_edit':
											urlLink = "ventana('win"+r+"', '&win_prp=win"+r+"&mod_edit=1&mod_tip=edit&prp_id="+prp_id+"&usr_id="+_USR_ID+"', 'system/prp_ficha_edit.php', 'Modificar Propiedad - ID:"+prp_id+"');";	
										break;
										case 'mod_del':
											urlLink = "ventana('win"+q+"', 'win_prp=win"+q+"&mod_edit=1&prp_id="+prp_id+"&usr_id="+_USR_ID+"', 'system/prp_ficha.php', 'Ficha de Propiedad');";
										break;
										case 'mod_search':
											urlLink = "ventana('win"+p+"', 'win_prp=win"+p+"&mod_edit=1&prp_id="+prp_id+"&usr_inmohost="+_USR_ID+"&usr_id="+prp_inmoID+"', 'system/prp_ficha.php', 'Ficha de Propiedad');";
										break;
										default:
										return false;
									}//switch
						...
						}else if(label == 'consulta_leida' || label == 'consulta_id') {
							...
							if(dato == '0') {
								...
								k=k+1;
								img ="";
								checkb="<input type=checkbox name='consulta_" + k +"' value='"+ consulta_id +"' id='consulta_"+k+"'>";
							...
							if(dato == '1') {
								title = "Editar Nota";
								img ="20/editar.png";
								urlLink="window.parent.window.ventana('edicionSimple', '&nota_id="+nota_id+"&usr_id="+_USR_ID+"', '"+fileNotasEdit+"', 'Editar Nota');";
							}
							else {
						...
						if(label != 'prp_inmoID' && label != 'pro_id'  && label != 'emp_id' && label!='cita_id' && label!='nota_id' && label != "" && label != "pub_id" ){ // agregar aca la cantidad de campos a ocultar
							...
							if(label == 'prp_dom' || label == 'nov_des') {
								...
								if(img != ""){ 
									...
									//Data += "<img src='../interfaz/inmohost/images/iconos/"+img+"' width='20' height='20' id='img"+i+"' /><center>"; ;
									Data += checkb+" </center>";
								}else if(label == 'consulta_leida'){
									
									if(dato == 1){Data = "<center>Si</center>"; }
                                                                }else if(label == 'prp_mostrar'){
		...
		if (indPrp<prpPagOffset)
		{
			arrPrp.length=indPrp;
			/*
			for (yy=indPrp;yy<prpPagOffset;yy++)
			{
				arrPrp[yy]=null;
			}
			*/
