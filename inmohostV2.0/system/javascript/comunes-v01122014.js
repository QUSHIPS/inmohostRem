...
function ver_foto(foto_addr)
{
	//alert("<img src='"+rutaAbsoluta+rutaSystemAbs+"php/funciones/foto.php?foto="+rutaAbsoluta+foto_addr+"&tam=700' border='0' />")
			...
			Dialog.alert("<img src='"+foto_addr+"' border='0' />", {
					 windowParameters: { className:"alert",  width:640,  height:540 }, 
					 okLabel: "Cerrar"
					 });
}
...
function verifEntreY(input_entre,input_y,desc){
	...
	return "";
	/*else if(isNaN(entre)){
			return "El valor debe ser num�rico en el Item "+desc + ", campo Entre";	
	}else{
			return "El valor debe ser num�rico en el Item "+desc + ", campo y";	
	}*/	
}
...
function contar_palabras(elemento){
		...
		cant_p = document.getElementById(elemento).value;
...
function cerrarSesion(usr_id){

	cambiarInterfaz('none');

      
	dialogos('cerrarSesion','','cerrarSesion','',usr_id);
...
function renovarSesion(){
	...
	var usuario=$('userLogin').value;
	var pass=$('passLogin').value;
        var usr_id=$('usuario_id').value;
	
	
	 var myAjax2 = new Ajax.Request(
								rutaSystema+"php/sec_chequea2.php",
								{method: 'get',
								parameters: 'usuario='+usuario+'&pass='+pass+'&usr_id='+usr_id,
...
function cambiar_estado_consulta(estado,consulta_id){

	var myAjax4 = new Ajax.Request(
					rutaSystema+"php/funciones/scripts_ajax.php",
					{method: 'get',
					asynchronous: false,
					parameters: "isAjax=1&estado="+estado+"&consulta_id="+consulta_id+"&op=13"+"&usr_id="+_USR_ID,
					onSuccess: function(transport) {
...
function marcar_consultas_leidas(emp_id,cantidad){
    
        document.getElementById('input_marcar_leidas').style.display='none';
        document.getElementById('img_precarga').style.display='';
	var cad_val;
	var vec_val;
	for(var j=1;j<=cantidad;j++){
		if($('consulta_'+j)){
			if($('consulta_'+j).checked){
				cad_val=$('consulta_'+j).value
				
				cambiar_estado_consulta('0',cad_val);
...
function eliminar_usuario(emp_id){ 	
		if (confirm("Si ELIMINA el usuario Seleccionado, se eliminaran todas las citas, novedades y referencias de este usuario en el sistema.\n\
                        Desea Continuar?"))
		{
...
function eliminar_bar_priv(bar_id,usr_id){ 	
		if (confirm("Esta seguro que desea eliminar el Barrio Privado seleccionado?"))
		{
			var myAjax4 = new Ajax.Request(
						rutaSystema+"php/funciones/scripts_ajax.php",
						{method: 'get',
						parameters: "isAjax=1&bar_id="+bar_id+"&op=10&usr_id="+usr_id,
						onSuccess: function(transport) { 
...
function muestra_progreso(){
	...
	div=document.getElementById("img_precarga");
	div.style.display='';
	
	div_input=document.getElementById("input_aceptar");
	div_input.innerHTML="Enviando Datos. Aguarde un instante por favor. ";
        
        
        div_input=document.getElementById("input_duplicar");
        if(div_input)
            div_input.innerHTML='';
}


function actualizar_redes(usr_id){
	
	var myAjax = new Ajax.Request(
					
					"actualizar_red.php?usr_id="+usr_id,
					...
					onSuccess: function(transport) { 
						...
						if(resultado==1){
							...
							$('pieMensajes').innerHTML='Se actualizo la red';
						}
				} 
			});
