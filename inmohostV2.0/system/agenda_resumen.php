...
<?php
	// Si viene por internet cambio las rutas para agregar Citas, Tasaciones Novedades y ver Telefonos
		...
		$fileTelefonos=_FILE_AGENDA_MEN_TELEFONO_NEW."&usr_id=$usr_id";
		$fileNovedad=_FILE_XML_NOVEDADES."?usr_login=".$_COOKIE['usuario']['usr_login']."&usr_id=$usr_id";
		$fileDemanda=_FILE_XML_DEMANDAS."&usr_id=$usr_id";
	
	$cadena="select emp_id from empleados where usr_login='".$_COOKIE['usuario']['usr_login']."'";
	$emp_id=mysql_query($cadena);
	$emp_id=mysql_result($emp_id,0,0);

	$menuItem_xsl = new MM_XSLTransform();
	$menuItem_xsl->setXML($fileRes."?usr_login=".$_COOKIE['usuario']['usr_login']."&usr_id=".$usr_id);
	...
	$menuItem_xsl->addParameter("fileConsultasXMLResultado", _FILE_XML_CONSULTAS_LISTADO);
	$menuItem_xsl->addParameter("fileConsultasXMLHead", _FILE_XML_CONSULTAS_HEAD);
	$menuItem_xsl->addParameter("fileNovedadNewPHP", _FILE_PHP_NOVEDADES."?usr_id=$usr_id");
	...
	$menuItem_xsl->addParameter("limite2", 0);
