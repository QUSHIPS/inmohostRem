...
<table width="240" border="0" align="center" cellpadding="0" cellspacing="0" id="tablaMorir">
 ...
 <?php	
	...
	/*	if ($internet)
		{
			$xml = str_replace($dominio_local,_DOMINIO_ACTUAL,_FILE_XML_RESUMEN_GENERAL);
			$filenov = str_replace($dominio_local,"localhost",_FILE_XML_NOVEDADES);
			$file_nov=$filenov;
			$file_nov.="?usr_login=".$usr_login;
		...
		$xsl = "../"._FILE_XSL_RESUMEN_FORMULARIO;	
		$file_nov=_FILE_XML_NOVEDADES;		
		$file_nov.="?usr_login=".$usr_login."&usr_id=".$usr_id;
		
		$id = "novedades";	
		$file[0] = _FILE_PHP_GENERICO_LISTADOS;
		$file[1] = _FILE_XML_NOVEDADES_HEAD."&usr_id=".$usr_id;
		$file[2] = _FILE_XML_NOVEDADES_LISTADO."&usr_id=".$usr_id;
...
?>
	...
	<input type="hidden" name="fileXMLListado" value="<?php echo _FILE_XML_NOVEDADES_LISTADO."&usr_id=".$usr_id; ?>"  />
	<input type="hidden" name="fileXMLHead" value="<?php echo _FILE_XML_NOVEDADES_HEAD."&usr_id=".$usr_id; ?>"  />
			<table width="100%" border="0" cellpadding="1" cellspacing="1" class="tableOscura" id="tablaListarNovedades" style="display:none">
              <tr class="tableClara">
				...
				<input id="desdeNovedadesAno" type="hidden" name="desdeNovedadesAno" value="" />
                  <a id="iniciaForm" href="javascript:;" title="seleccionar fecha" class="menu" onclick="position(event); toolTip('<?php echo _FILE_PHP_MEN_CALENDARIO."&usr_id=".$usr_id; ?>&destino=desdeNovedades',this)" tabindex="1"><img src="interfaz/inmohost/images/iconos/calendario.gif" width="16" height="15" border="0" align="absmiddle" /></a></td>
              </tr>
              <tr class="tableClara">
				...
				<input id="hastaNovedadesAno" type="hidden" name="hastaNovedadesAno" value="" />
                  <a href="javascript:;" title="seleccionar fecha" class="menu" onclick="position(event); toolTip('<?php echo _FILE_PHP_MEN_CALENDARIO."&usr_id=".$usr_id;?>&destino=hastaNovedades',this)" tabindex="2"><img src="interfaz/inmohost/images/iconos/calendario.gif" width="16" height="15" border="0" align="absmiddle" /></a></td>
              </tr>
              <tr class="tableClara">
