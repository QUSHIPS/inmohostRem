 ...
 switch ($op) {
     ...
     case "modi":
			...
			if($priv_id!=1){
				...
				echo "<b>NO TIENE PRIVILEGIOS PARA UTILIZAR ESTA FUNCION</b>";
				exit;
			}
...
<table width="280" border="0" align="center" cellpadding="0" cellspacing="0" id="tablaMorir">
  <tr>
    <td><form id="prpFormBuscar" name="prpFormBuscar" method="post" action="" >
	 ...
	 <input type="hidden" name="usr_id"  value="<?php echo $_COOKIE['usuario']['usr_id'];?>"/>
<?php 
	if ($op2=="modi")
	{
...
?>
	 <input type="hidden" name="usr_id_sys"  value="<?php echo $_COOKIE['usuario']['usr_id'];?>"/>
	 
      <table width="100%" border="0" cellspacing="0" cellpadding="1">
        <tr>
          <td colspan="2"><h1 align="left"><?php echo $titulo;?></h1><hr /></td>
          </tr>
       <?php if($op!="modi" && $op!="del" ){ ?>
             <tr>
              ...
              <td>
                  <div align="left">
                    ...
                    <option value="-2">Todos los Inmuebles de la RED </option>
                    <?php
                                             /* REGS= ID,Descripcion  */
                                            $regs=" usuarios.usr_id,usuarios.usr_raz ";
                                            $tablas=" usuarios left join propiedades on usuarios.usr_id = propiedades.usr_id";
                                            $id=_USR_ID;
                                            $xtras=" order by usr_raz ASC";
                                            $db = "inmoclick_database";
                                            $filtro = "visible =1 and usr_tipo = 2 and propiedades.prp_mostrar = 1 and propiedades.mos_por_1 and (plan_usuario_id in (2,3,4,7,8) or plan_usuario_id is null) and usuarios.pro_id = 21 group by usuarios.usr_id";
                    include("../"._FILE_SCRIPT_PHP_SELECT);
                      ...
                      <input type="hidden" name="buscador_avanzado" value="1">
              </div></td>
            </tr> 
        <?php }else{
        ?>
            <input type="hidden" name="inmo_id" value="<?php echo _USR_ID?>">
		...
		<tr>
          ...
          <td><div align="left"><select style="width:190px" name='estado' class="inputForm"  id='estado'>
          			...
          			<option value='10'>Pre/Cargada</option></select>
          </div></td>
		</tr>              
        ...
        <tr>
          ...
          <td>          
              <div align="left">
                <select style="width:100px" name="offset" class="inputForm" id="offset" tabindex="12" onkeypress="if (event.keyCode == 13){$('prp_id').value=''; chequeaForm('prpFormBuscar', '<?php echo $destino1; ?>', '<?php echo $titulo; ?>', '<?php echo $url1; ?>', '<?php echo $parametros; ?>');}" >
